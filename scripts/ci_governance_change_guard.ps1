...
$hasJust=$false; foreach($f in $chgEff){ if($f -match '^docs/governance/GOVERNANCE_CHANGE_(PR\d+|\d{8}T\d{6}Z)\.md$'){ $hasJust=$true; break } }
if($gov -and -not $hasJust){ Write-Error "GOV_CHANGE_GUARD FAIL: governance touched; missing docs/governance/GOVERNANCE_CHANGE_PR<UTC>.md (must be included in PR changes)"; exit 1 }
