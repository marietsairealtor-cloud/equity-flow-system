...
### **10.8.12 — Automate Cloud Migration Parity Registry**

**Deliverable:**
Convert `docs/truth/cloud_migration_parity.json` from hand-authored to machine-derived. Eliminate manual migration tip/count bumps on every migration PR.

**DoD:**

- `cloud_migration_parity.json` is regenerated by `npm run handoff` from `supabase/migrations/**` directory contents
- Generator reads migration filenames, counts total, identifies latest (lexicographic sort), writes JSON
- Running `npm run handoff` twice produces identical output (deterministic)
- §3.0.4c exemption removed — file is now machine-derived and included in handoff regeneration surface
- `handoff.ps1` updated to include `cloud_migration_parity.json` in generation surface
- Truth-bootstrap validates the file (already registered)
- Robot-owned guard protects the file (already registered)
- CI fails if `cloud_migration_parity.json` differs from what handoff would produce (same pattern as schema drift)
- Manual edits to `cloud_migration_parity.json` are forbidden after this item merges — handoff is the only writer
- pgTAP not required (no DB change)
- SOP_WORKFLOW.md §16 updated: migration changes listed as a handoff trigger now also regenerate cloud_migration_parity.json (no separate manual step)

**Proof:** `docs/proofs/10.8.12_automate_migration_parity_<UTC>.log`

**Gate:** `lane-only` (automation improvement, no new enforcement surface)
