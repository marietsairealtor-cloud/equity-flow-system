...
| send_offer_v1 | 10.13B, 10.13B1 | Atomic **`analyzing` -> `offer_sent`** with prerequisite **`deal_soft_offers`** row; **`deal_reminders`** (**`offer_follow_up`**, **`clock_timestamp() + interval '3 days'`**); **`deal_activity_log`** (**`stage_change`**, user-facing copy **`Offer sent to seller`** per **10.13B1**); **`rpc_idempotency_log`** replay + atomic claim (**`RETURNING (xmax = 0)`**) before mutations | SECURITY DEFINER, authenticated only, min role member, workspace write lock, **VOLATILE** | **`current_tenant_id()`** + **`auth.uid()`** — **`p_deal_id`**, **`p_idempotency_key`** |
