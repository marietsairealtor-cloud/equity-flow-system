...
| handoff_to_tc_v1 | 10.11A; activity log **10.14A**; milestones + notification **10.14B** | `dispo` → `tc`; sets `assignee_user_id`; **`require_min_role_v1('member')`** first; requires **`auth.uid()`** + tenant; **`assignment_agreement_signed_at`** and **`earnest_money_received_at`** both required (**`CONFLICT`** + **`error.fields`** if missing); on success writes **`handoff`** **`Deal handed off to TC`** to **`deal_activity_log`**; non-null assignee inserts **`workspace_handoff_notifications`** (**`handoff_to_tc`**) | SECURITY DEFINER, min role: member | non-NULL `current_tenant_id()` and `auth.uid()` — `p_deal_id` + `p_assignee_user_id` |
| return_to_acq_v1 | 10.11A; activity log **10.14B** | dispo → under_contract (undo dispo); **`require_min_role_v1('member')`** first; **`auth.uid()`** + tenant; on success writes **`handoff`** **`Deal returned to Acq from Dispo`** to **`deal_activity_log`** | SECURITY DEFINER, min role: member | non-NULL `current_tenant_id()` and `auth.uid()` — `p_deal_id` only |
| return_to_dispo_v1 | 10.11A | tc → dispo (undo tc) | SECURITY DEFINER, min role: member | current_tenant_id() — p_deal_id only |
| delete_deal_document_v1 | 10.14B5A | Soft-delete document metadata; sets deleted_at and deleted_by; storage file deletion out of scope | SECURITY DEFINER, authenticated only, min role: member, workspace write-lock | `p_document_id uuid` — tenant from `current_tenant_id()` |
  | attach_deal_document_v1 | 10.14B5 | Record governed document metadata after client uploads file to Supabase Storage; allowed document_type: signed_purchase_agreement; storage_path validated against {tenant_id}/{deal_id}/documents/{document_type}/{filename} convention | SECURITY DEFINER, authenticated only, min role: member, workspace write-lock | current_tenant_id() — p_deal_id + p_document_type + p_storage_path + p_file_name + p_mime_type + p_file_size |
| list_deal_documents_v1 | 10.14B5 | List document metadata for a deal; metadata only, no file contents | SECURITY DEFINER, STABLE, authenticated only, min role: member | current_tenant_id() — p_deal_id only |
...
| refresh_deal_soft_offer_v1 | 10.13A, 10.13C-D | **`require_min_role_v1('member')`** first executable statement; then **`auth.uid()`** + non-empty key; **`rpc_idempotency_log`** replay (**verbatim `result_json`**) **before** tenant/workspace/`get_offer_payload_v1`/writes; persists **`deal_soft_offers`** + atomic **`rpc_idempotency_log`** claim (**`ON CONFLICT`** loser replay); ASCII hyphen email subject **`Soft offer -`**. **10.13C-D:** required first call in **Offer Sent** UI workflow before **`send_offer_v1`** | SECURITY DEFINER, authenticated only, min role member, workspace write lock, **VOLATILE** | **`current_tenant_id()`** + **`auth.uid()`** — **`p_deal_id`**, **`p_idempotency_key`** |
| send_offer_v1 | 10.13B, 10.13B1, 10.13C-D | Atomic **`analyzing` -> `offer_sent`** with prerequisite **`deal_soft_offers`** row; **`deal_reminders`** (**`offer_follow_up`**, **`clock_timestamp() + interval '3 days'`**); **`deal_activity_log`** (**`stage_change`**, user-facing copy **`Offer sent to seller`** per **10.13B1**); **`rpc_idempotency_log`** replay + atomic claim (**`RETURNING (xmax = 0)`**) before mutations. **10.13C-D:** sole governed **Send Offer** mutation on Acquisition UI — do not pair with **`advance_deal_stage_v1(..., 'send_offer')`** | SECURITY DEFINER, authenticated only, min role member, workspace write lock, **VOLATILE** | **`current_tenant_id()`** + **`auth.uid()`** — **`p_deal_id`**, **`p_idempotency_key`** |
...
| create_deal_note_v1 | 10.11A1 / 10.14B4C | Append user note on `deal_notes` for a deal; `note_type = call_log` rejected in 10.14B4C -- no governed UI writes call logs; historical call_log rows remain readable | SECURITY DEFINER, min role: member | current_tenant_id() — p_deal_id + p_note_type + p_content |
...
| lookup_share_token_v1 | 6.7/8.7/8.10; **10.14B** regression | Look up share token by token + deal_id scope; logs attempt (best-effort, hash-only). deal_id required (8.10). **`NOT_FOUND`** envelope identical for invalid / expired / revoked (no leak). | SECURITY DEFINER | current_tenant_id() - no tenant_id param |
| revoke_share_token_v1 | 8.6; **10.14B** regression | Revoke a share token immediately (idempotent) | SECURITY DEFINER | current_tenant_id() — no tenant_id param |
| create_share_token_v1 | 8.8/8.9; **10.14B** regression | Generate cryptographically secure share token (shr_ prefix, 256-bit entropy, hash-at-rest); expires_at required (8.9) | SECURITY DEFINER | current_tenant_id() — no tenant_id param |
...
| set_dispo_deal_milestone_v1 | 10.14B2 | Set or clear assignment_agreement_signed_at / earnest_money_received_at on a Dispo deal; writes activity log on mutation | SECURITY DEFINER, authenticated only, min role: member, workspace write lock, Dispo stage only | `p_deal_id uuid`, `p_milestone text`, `p_is_complete boolean` — tenant from `current_tenant_id()` |
  | update_buyer_active_status_v1 | 10.14B1 | Activate or deactivate a persisted buyer in `intake_buyers` for current tenant | SECURITY DEFINER, authenticated only, min role: member, workspace write lock | `p_buyer_id uuid`, `p_is_active boolean` — tenant from `current_tenant_id()` |
...
| `list_acq_deals_v1(p_filter text, p_farm_area_id uuid)` | List active Acquisition pipeline deals; `p_filter` ∈ `all`, `new`, `analyzing`, `offer_sent`, `under_contract`, `follow_ups` (reminder-driven). **10.14B4B:** `next_action` and `next_action_due` removed from `data.items`. |
| `get_acq_deal_v1(p_deal_id uuid)` | Full detail for one deal including embedded `properties` and latest `pricing` from `deal_inputs` (`pricing` includes `mao` and `multiplier` — 10.11A3). Top-level `last_contacted_at` from the most recent `call_log` on `deal_notes`, or `null` if there is no call log. **10.13E:** governed **reopen** read for ACQ deal detail after **`update_deal_pricing_v1`** save (same `pricing` contract). No new RPC; no schema changes. **10.14B3:** `properties` object now includes `electrical` and `plumbing` (text, nullable). **10.14B4B:** `next_action` and `next_action_due` removed from output. **10.14B4C:** `last_contacted_at` removed from output. |
...
| `handoff_to_dispo_v1` / `handoff_to_tc_v1` | Stage handoffs with assignee; wrong stage → `CONFLICT`. **`handoff_to_dispo_v1` (10.11A10):** successful path appends **`handoff`** to **`deal_activity_log`**; requires tenant + user JWT context (**§62**). **`handoff_to_tc_v1` (10.14A):** same tenant + user requirement; successful path appends **`handoff`** with content **`Deal handed off to TC`** (**§71**). **`handoff_to_tc_v1` (10.14B):** both **`assignment_agreement_signed_at`** and **`earnest_money_received_at`** required on **`deals`** before handoff (**`CONFLICT`** + **`error.fields`** otherwise); non-null assignee inserts **`workspace_handoff_notifications`** (**§72**). |
| `return_to_acq_v1` / `return_to_dispo_v1` | Reverse handoffs (`dispo`→`under_contract`, `tc`→`dispo`). **`return_to_acq_v1` (10.14B):** **`require_min_role_v1('member')`** + **`auth.uid()`**; successful path appends **`handoff`** **`Deal returned to Acq from Dispo`** (**§72**). |
...
- **Allowed keys in `p_fields`:** `property_type`, `beds`, `baths`, `sqft`, `lot_size`, `year_built`, `occupancy`, `deficiency_tags`, `condition_notes`, `garage_parking`, `basement_type`, `foundation_type`, `roof_age`, `furnace_age`, `ac_age`, `heating_type`, `cooling_type`, `electrical`, `plumbing` (added **10.14B3** — operator-captured, not from public seller form).
...
### `handoff_to_tc_v1` (10.14B overlay)

- **Authority overlay:** migration **`20260516000001_10_14B_dispo_share_link_handoff.sql`** further replaces **`handoff_to_tc_v1`**: **`SELECT`** loads **`assignment_agreement_signed_at`** and **`earnest_money_received_at`**; if either is **`NULL`** while stage is **`dispo`**, returns **`CONFLICT`** with **`error.fields`** listing missing milestone keys (**`required`** markers). On success with non-**`NULL`** **`p_assignee_user_id`**, **`INSERT`** into **`workspace_handoff_notifications`** (**`kind = handoff_to_tc`**) for downstream notification delivery.

### `return_to_acq_v1` (10.14B overlay)

- **Authority overlay:** same migration replaces **`return_to_acq_v1`** with **`require_min_role_v1('member')`** first (**`NOT_AUTHORIZED`** on guard failure), **`current_tenant_id()`** + **`auth.uid()`** required, **`check_workspace_write_allowed_v1()`** before write. Successful **`dispo` → `under_contract`** appends **`deal_activity_log`** (**`activity_type = handoff`**, **`content = Deal returned to Acq from Dispo`**, **`created_by = auth.uid()`**).
...
**§Registry:** `docs/truth/rpc_contract_registry.json` — **`advance_deal_stage_v1`**, **`handoff_to_dispo_v1`**, **`handoff_to_tc_v1`** (overlay **10.14A** + **10.14B**), **`return_to_acq_v1`** (**10.14B**), **`complete_reminder_v1`** **`build_route_owner`** **10.11A10** / **10.14A** / **10.14B** as applicable.
...
- **10.13C-D — Offer Sent + Email Offer:** **Offer Sent** ( **`Analyzing`** only ) calls **`refresh_deal_soft_offer_v1`** then **`send_offer_v1`** in that order, each with its own non-empty **`p_idempotency_key`**, then refreshes list/detail/activity/reminders per **`docs/ui-workflows/WORKFLOWS.md`**. Do **not** call **`advance_deal_stage_v1(..., 'send_offer')`** for that product path. **Email Offer** opens a native **`mailto:`** link only (seller email, property address in subject, seller-ready body); optional **`get_offer_payload_v1`** may supply body copy when explicitly bound in the workflow registry — no SMTP, no in-app send, no PDF attachment for this Build Route item.
...
**§Registry:** UI truth — **`docs/ui-workflows/WORKFLOWS.md`** (**`acq-offer-sent`**, **`acq-email-offer`** — **10.13C-D**); RPC truth — **`docs/truth/rpc_contract_registry.json`** (Build Route backend owners unchanged by this subsection except cross-refs in **§17** rows above).

---

## 64) Intake Backend — Submission Persistence (10.12A)

**Authority:** migration **`20260503000001_10_12A_intake_submission_persistence.sql`**. **Lead Intake KPI read path:** **`20260505000002_10_12C2_lead_intake_kpis.sql`** (`get_lead_intake_kpis_v1` baseline); **10.12C4** **`20260507000001_10_12C4_submission_review_outcomes.sql`** extends KPI payload + review columns + **`mark_submission_reviewed_v1`** + inbox filter + draft promotion trigger; **10.12C5** **`20260508000001_10_12C5_kpi_unreviewed_count_fix.sql`** recreates **`get_lead_intake_kpis_v1`** only to narrow **`unreviewed_count`** to **seller** + **birddog** with **`review_status = unreviewed`**. **Draft read for Lead Intake UI pre-fill:** **`20260509000001_10_12C6_get_draft_deal_read_path.sql`** introduces **`get_draft_deal_v1(p_draft_id uuid)`**. **10.12C8** **`20260511000001_10_12C8_mark_submission_reviewed_draft_id.sql`** replaces **`mark_submission_reviewed_v1`** with canonical **`(p_outcome text, p_submission_id uuid DEFAULT NULL, p_draft_id uuid DEFAULT NULL)`** plus legacy **`(p_submission_id uuid, p_outcome text)`** forwarder (**`LANGUAGE plpgsql`**, **`PERFORM public.current_tenant_id()`** for definer-safety-audit). **`20260512000001_10_12C8_legacy_wrapper_definer_audit.sql`** idempotently **`CREATE OR REPLACE`**s the legacy overload if an earlier SQL-only wrapper was applied. **Lead Intake inbox list filter:** **`20260506000001_10_12C3_list_intake_submissions_filter.sql`** (`list_intake_submissions_v1` seller/birddog-only); **10.12C4** narrows list to **`review_status = unreviewed`** and adds review fields to list payload.
...
- **Metrics (OK `data`, 10.12C4 + 10.12C5 queue fix):** **`new_submissions`** — count of **seller + birddog** rows in window. **`new_leads`** — same scope **excluding** rows whose **`review_outcome`** is **`rejected_spam`**, **`rejected_test`**, or **`rejected_invalid`** (buyer rows still excluded from both counts). **`rejected_count`** — seller + birddog in window with those three reject outcomes. **`submission_to_deal_pct`** — integer percent; **denominator = `new_leads`**; **numerator** — in-window seller/birddog rows that count toward leads and are **promoted** (**`review_outcome = promoted`** or linked **`draft_deals.promoted_deal_id IS NOT NULL`**); zero denominator ⇒ **0**. **`avg_review_time_hours`** — one decimal; rows in window with **`review_status = reviewed`**, **`reviewed_at` IS NOT NULL**; none ⇒ **0**. **`unreviewed_count`** — **all-time** tenant rows with **`review_status = unreviewed`** **and** **`form_type`** **seller** or **birddog** only (**10.12C5**; not window-filtered; **buyer** unreviewed rows excluded). Echoes **`date_from`**, **`date_to`** (effective bounds).
...
### `mark_submission_reviewed_v1` → `jsonb`

- **Authority / migration:** **`20260507000001_10_12C4_submission_review_outcomes.sql`** introduced **`mark_submission_reviewed_v1(p_submission_id uuid, p_outcome text)`**. **10.12C8** **`20260511000001_10_12C8_mark_submission_reviewed_draft_id.sql`** replaces with canonical **`mark_submission_reviewed_v1(p_outcome text, p_submission_id uuid DEFAULT NULL, p_draft_id uuid DEFAULT NULL)`** (required **`p_outcome`** first so **PostgreSQL** default-parameter rules hold) and a **`LANGUAGE plpgsql`** compatibility overload **`mark_submission_reviewed_v1(p_submission_id uuid, p_outcome text)`** that **`PERFORM public.current_tenant_id()`** then **`RETURN`**s canonical **`(p_outcome, p_submission_id, NULL::uuid)`** (definer-safety-audit **6.2** **prosrc** tenant touch). **`20260512000001_10_12C8_legacy_wrapper_definer_audit.sql`** is idempotent **`CREATE OR REPLACE`** of that overload for databases that applied an earlier SQL-only wrapper. **`GRANT EXECUTE`** on both overloads to **`authenticated`** only (`REVOKE` from `anon`, `PUBLIC`).
...
- **Identifiers (10.12C8 canonical):** Neither **`p_submission_id`** nor **`p_draft_id`** ⇒ **`VALIDATION_ERROR`**. If **`p_submission_id`** is set, it wins (**`p_draft_id`** ignored for row selection). Else resolve by **`intake_submissions.draft_deals_id = p_draft_id AND tenant_id = current tenant`**; no row ⇒ **`NOT_FOUND`**. Primary row fetch uses **`id AND tenant_id`** (no cross-tenant leak).
- **Scope:** **seller** and **birddog** rows only; **buyer** ⇒ **`VALIDATION_ERROR`**. Unknown or non-tenant row ⇒ **`NOT_FOUND`**.
...
**§Registry:** **`docs/truth/rpc_contract_registry.json`** — **`get_dispo_kpis_v1`**, **`list_dispo_dashboard_deals_v1`**, **`handoff_to_tc_v1`** notes; **`docs/truth/write_path_registry.json`**; **`tenant_table_selector.json`**; **`§17`** mapping table above; full **`handoff_to_tc_v1`** / **`return_to_acq_v1`** / share-token **`NOT_FOUND`** parity — **§72**.

---

## 72) Dispo backend — share link + handoff control (10.14B)

**Authority:** migration **`20260516000001_10_14B_dispo_share_link_handoff.sql`**.

**Purpose:** Gate **`dispo` → `tc`** on recorded assignment and deposit milestones; persist assignee handoff signals; align **`return_to_acq_v1`** with member + JWT user guards and **`deal_activity_log`** audit. Share link **create / revoke / lookup** implementations remain on prior migrations (**8.x**, **9.x**, **10.8.11N**); **10.14B** adds **`supabase/tests/10_14B_dispo_share_link_handoff.test.sql`** for lane regression (**`NOT_FOUND`** envelope parity).

### Schema

- **`deals.assignment_agreement_signed_at`**, **`deals.earnest_money_received_at`** (nullable **`timestamptz`**): both must be non-**`NULL`** before **`handoff_to_tc_v1`** performs the stage update; else **`CONFLICT`** with **`error.fields`** echoing missing keys.
- **`workspace_handoff_notifications`**: append-only (**`tenant_id`**, **`recipient_user_id`**, **`deal_id`**, **`kind = handoff_to_tc`**). **No** **`GRANT`** to **`anon`** / **`authenticated`**; **`SECURITY DEFINER`** inserts only.

### `handoff_to_tc_v1` (**10.14B** delta)

- After **§62** / **§71** / **10.14A** guards, **`SELECT`** loads milestone timestamps; missing either on a **`dispo`** deal -> **`CONFLICT`**. Success path: existing **`UPDATE deals`** + **`INSERT deal_activity_log`**; if **`p_assignee_user_id`** is set, **`INSERT workspace_handoff_notifications`**.

### `return_to_acq_v1` (**10.14B** delta)

- **`require_min_role_v1('member')`** first; **`current_tenant_id()`** + **`auth.uid()`**; **`check_workspace_write_allowed_v1()`**; success **`INSERT deal_activity_log`** (**`Deal returned to Acq from Dispo`**).

### Share token RPCs (signatures unchanged)
