...
## pgTAP + Migration Authoring Discipline (ENFORCED)

### Authoring Law (LOCKED)

The migration is truth. The test verifies truth. This order is non-negotiable and applies to every migration + test PR without exception:

29A. Write the migration. It is the system of record.
29B. Apply the migration to a clean DB. If it fails, fix the migration.
29C. Write the pgTAP test to verify what the migration claims. The test must fail if the migration is absent or wrong.
29D. If a test fails after the migration is applied, the migration is wrong. Fix the migration. The test is frozen until the migration is correct.
29E. A test that passes without the migration present is not a test — it is dead code.

### Test File Naming Rule (ENFORCED)

29F. Every pgTAP test file must be named to exactly match its paired migration with the timestamp prefix stripped and `.test.sql` appended:

* Migration: `20260318000001_10_8_3_reminder_engine.sql`
* Required test: `10_8_3_reminder_engine.test.sql`

The item ID prefix is mandatory. One extension only: `.test.sql` — no `.pgtap.sql` or bare `.sql`.

Pairing is deterministic: strip the timestamp from the migration filename, append `.test.sql`. That is the required test filename. No deviation permitted.

### Test Authoring Rules (ENFORCED)

29G. Every RPC behavioral test must call the RPC and inspect its return value. A test labelled as verifying an RPC that queries the underlying table directly instead is a violation — it verifies nothing about the RPC contract.

29H. Every tenant isolation test must use a distinct second tenant with its own seed data. Cross-tenant isolation must be proven by attempting the action as tenant 2 and verifying the negative outcome — not by asserting a state already set by tenant 1.

29I. Every state-change RPC (INSERT, UPDATE) must have a post-call state verification via a read RPC or explicit assertion — not by assuming success based on `ok=true` alone.

29J. Plan count declared in `plan(n)` must match the actual number of test calls exactly. Count every `SELECT is(`, `SELECT isnt(`, `SELECT ok(`, `SELECT has_table(`, `SELECT has_column(`, `SELECT throws_ok(` call. Mismatch is a violation.

29K. Every test file must be wrapped in `BEGIN` / `ROLLBACK`. No test may leave persistent state.

### Corrective Migration Exception

29L. Corrective migrations that fix encoding, comments, or other non-schema-surface issues with no behavioral change are intentionally unpaired. No test file is required. These must be documented as UNPAIRED-CORRECTIVE in any audit log. Examples: `_fix_comment_encoding.sql`, `_fix_encoding.sql`.

### Violation Consequence (LOCKED)

29M. If a coder modifies a test to achieve a passing CI run without correcting the underlying migration, this is a governance violation. The PR must be reverted. A stop-the-line entry is required in `docs/threats/INCIDENTS.md`. The branch may not be reopened until QA has reviewed and approved a remediation plan. This rule is enforced at the same level as §25–28.
