...
-- send_offer_v1(p_deal_id, p_idempotency_key): atomic stage advance analyzing -> offer_sent,
-- follow-up reminder insert (same persistence surface as create_reminder_v1), activity log row,
-- prerequisite persisted soft-offer row (deal_soft_offers). Replay via rpc_idempotency_log.
