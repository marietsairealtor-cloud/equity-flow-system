...
-- Adds dispo_* packet fields to deals table.
-- Adds update_dispo_packet_v1: governed authenticated mutation using p_fields jsonb patch.
-- Adds lookup_share_token_public_v1: new anon-callable public buyer-facing RPC (does not modify lookup_share_token_v1).

-- 1. Add dispo packet fields to deals table
ALTER TABLE public.deals
  ADD COLUMN dispo_asking_price          numeric      NULL,
  ADD COLUMN dispo_intersection          text         NULL,
  ADD COLUMN dispo_closing_date          date         NULL,
  ADD COLUMN dispo_description           text         NULL,
  ADD COLUMN dispo_comparables           text         NULL,
  ADD COLUMN dispo_media_url             text         NULL,
  ADD COLUMN dispo_market_value_estimate numeric      NULL,
  ADD COLUMN dispo_below_market_override numeric      NULL;
...
-- 2. update_dispo_packet_v1
-- p_fields jsonb patch: only keys present are updated. Explicit null clears field. Omitted = unchanged.
...
-- 3. lookup_share_token_public_v1: new public buyer-facing lookup (anon-callable)
-- lookup_share_token_v1 remains unchanged (authenticated, tenant-scoped).
-- Token is the authorization mechanism. No tenant context required.
-- Returns only allowlisted buyer-facing packet fields. No exact address. No seller fields.
-- Identical NOT_FOUND envelope for all failure cases (no existence leak).
