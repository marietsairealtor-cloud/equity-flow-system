-- Migration: 10.8.8A -- Create Workspace RPC
-- Adds create_tenant_v1(p_idempotency_key text) for workspace creation in onboarding.
-- SECURITY DEFINER; authenticated-only; no caller-supplied tenant_id.
-- Idempotency: atomic claim via unique constraint on rpc_idempotency_log.

DROP FUNCTION IF EXISTS public.create_tenant_v1();
...
-- Required by definer-safety-audit gate
