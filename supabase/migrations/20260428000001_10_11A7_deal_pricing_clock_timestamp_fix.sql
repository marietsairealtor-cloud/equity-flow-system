-- 10.11A7 corrective: use clock_timestamp() instead of now() for deterministic row ordering
-- now() returns transaction timestamp -- same value for all calls in same transaction
-- clock_timestamp() returns wall clock time -- unique per call
-- This ensures ORDER BY created_at DESC, id DESC is deterministic when multiple rows
-- are inserted in the same transaction during tests
