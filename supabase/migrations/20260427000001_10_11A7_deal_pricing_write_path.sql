...
-- Updates deals.assumptions_snapshot_id to point to new row
-- Allowed keys: arv, ask_price, repair_estimate, mao, multiplier
-- All fields are numeric -- validated safely before insert
-- Missing base deal_inputs row = NOT_FOUND (no auto-create)
-- explicit null = remove key from assumptions (not JSON null)

CREATE OR REPLACE FUNCTION public.update_deal_pricing_v1(
  p_deal_id uuid,
  p_fields  jsonb
)
RETURNS json
